...
#else
    __shared__ float smem[THREADS_NUM];
#endif

    data[tid] = blockScanInclusive<THREADS_NUM>(data[tid], smem, tid);
}

#define BLOCK_SCAN_FLOAT_TEST(block_size)                                   \
    TEST(BlockScanFloat, BlockSize##block_size)                             \
    {                                                                       \
        Mat src = randomMat(Size(block_size, 1), CV_32FC1, 0.0, 1.0);       \
                                                                            \
        GpuMat d_src;                                                       \
        d_src.upload(src);                                                  \
                                                                            \
        for (int col = 1; col < block_size; col++)                          \
            src.at<float>(0, col) += src.at<float>(0, col - 1);             \
                                                                            \
        float_kernel<block_size><<<1, block_size>>>((float*)d_src.data);    \
                                                                            \
        CV_CUDEV_SAFE_CALL(cudaDeviceSynchronize());                        \
                                                                            \
        EXPECT_MAT_NEAR(d_src, src, 5e-3);                                  \
    }

BLOCK_SCAN_FLOAT_TEST(29)
BLOCK_SCAN_FLOAT_TEST(30)
BLOCK_SCAN_FLOAT_TEST(32)
BLOCK_SCAN_FLOAT_TEST(40)
BLOCK_SCAN_FLOAT_TEST(41)

BLOCK_SCAN_FLOAT_TEST(59)
BLOCK_SCAN_FLOAT_TEST(60)
BLOCK_SCAN_FLOAT_TEST(64)
BLOCK_SCAN_FLOAT_TEST(70)
BLOCK_SCAN_FLOAT_TEST(71)

BLOCK_SCAN_FLOAT_TEST(109)
BLOCK_SCAN_FLOAT_TEST(110)
BLOCK_SCAN_FLOAT_TEST(128)
BLOCK_SCAN_FLOAT_TEST(130)
BLOCK_SCAN_FLOAT_TEST(131)

BLOCK_SCAN_FLOAT_TEST(189)
BLOCK_SCAN_FLOAT_TEST(200)
BLOCK_SCAN_FLOAT_TEST(256)
BLOCK_SCAN_FLOAT_TEST(300)
BLOCK_SCAN_FLOAT_TEST(311)

BLOCK_SCAN_FLOAT_TEST(489)
BLOCK_SCAN_FLOAT_TEST(500)
BLOCK_SCAN_FLOAT_TEST(512)
BLOCK_SCAN_FLOAT_TEST(600)
BLOCK_SCAN_FLOAT_TEST(611)

BLOCK_SCAN_FLOAT_TEST(1024)
